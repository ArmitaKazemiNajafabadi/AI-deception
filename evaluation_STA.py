...
def evaluate_one_episode(cyborg, wrapped_cyborg, agent, write_to_file, i,tot):
    ...
    total_reward = sum(r)
    ## I added this to evaluate un
    # print("NUUL")
    # ssum = 0
    # for agent_name, agent in submission.AGENTS.items():
    #     ssum += agent.used
    #     agent.used = 0
    # print(f"all agents used {ssum}")
...
def run_evaluation(submission, log_path, max_eps=100, write_to_file=False, seed=None):
    ...
    total_reward = []
    spatial_used = np.zeros((max_eps, EPISODE_LENGTH))
    total_used = []
    actions_log = []
    obs_log = []
    for i in tqdm(range(max_eps)):
        ...
        for j in range(EPISODE_LENGTH):
            ...
            for agent_name, agent in submission.AGENTS.items():
                ssum += agent.used
                agent.used = 0
            spatial_used[i,j] = ssum
            
        print(spatial_used[i,:])
            
        total_reward.append(sum(r))
        print(f"summed reward: {sum(r)}")
    ...
    print(reward_string)
    np.save("STAttck_spatial_used_list_080", spatial_used, allow_pickle=True)
