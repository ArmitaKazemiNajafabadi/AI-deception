...
class STGraphWrapper(EnterpriseMAE):
    ...
    def reset(self):
        ...
        if self.reset_count > 98:
            np.save("STAttck_action_def_list_080.npy", self.action_def_list, allow_pickle=True)
            np.save("STAttck_reward_list_080.npy", self.reward_list, allow_pickle=True)
            np.save("STAttck_budget_080.npy", self.budget, allow_pickle=True)
