...
class InductiveGraphPPOAgent():
    ...
    def get_action(self, obs, *args):
        ...
        threshold = 0.2  # Or whatever threshold you choose
        
        # probs = distro.probs.clone()  # Clone to avoid modifying original distro
        probs = distro.probs.squeeze().clone()
        max_prob, max_idx = probs.max(dim=0)
        # print(max_prob)
        # print(env)
        if (max_prob.item() > threshold):
            # print("in loop")
            # print(f"Boosting distribution (max_prob={max_prob.item():.2f})")
            # print(f"Previous Distribute {probs}")
            num_actions = probs.shape[0]
            # Add max_prob to all actions (bias boost)
            boosted_probs = probs + (max_prob.item()/(num_actions-1))*(6/8)
            
            boosted_probs[max_idx] = max_prob.item()*(2/8)
        
            # Create new distribution from modified probabilities
            distro = torch.distributions.Categorical(probs=boosted_probs)
            # print(f"New Distribute: {distro.probs.squeeze()}")
            self.used += 1
            # env.M -= 1
            # print("self.used: " + str(self.used))
        # Now do normal deterministic/sample logic
        if self.deterministic:
            action = distro.probs.argmax()
            print("deterministic (after boost)!")
        else:
            action = distro.sample()



        # print("DistribuProbs:", distro.probs.max())
        ...
        # return action.item(), value.item(), prob.item(), budget
